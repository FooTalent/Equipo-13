        ...
        this.userService.resetPassword(resetData).subscribe({
          next: (response) => {
            const dialogRef = this.dialog.open(MensajeDialogoComponent,{
              data: { title: 'Contraseña restablecida con éxito', content: 'Ya puedes volver a iniciar sesión con tu nueva contraseña', isSuccess:true}
            });
          ...
          error: (error) => {
            const dialogRef = this.dialog.open(MensajeDialogoComponent,{
              data: { title: 'Error', content: 'Ha ocurrido un error al restablecer la contraseña. Por favor, inténtalo de nuevo.', redirectTo:'/error', isSuccess:false }
            });
            dialogRef.afterClosed().subscribe(() => {
